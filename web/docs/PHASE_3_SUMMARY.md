...
- **POST /api/chores** — Create a chore with `title`, `frequency`, optional `description`, and optional `assigneeIds`. Validates all inputs. Returns 201.
...
- **PUT /api/chores/[id]** — Partial update of `title`, `frequency`, `description`, `assigneeIds`. Uses transaction for atomic assignment replacement. Returns 404 if not found.
...
### Completions API
- **POST /api/completions** — Record a completion for a chore. Uses session user as completing user. Optional `scheduleId`, `notes`, `completedAt`. Validates chore/schedule existence. Returns 201.
...
├── package.json                            # MODIFIED: prisma.seed config
